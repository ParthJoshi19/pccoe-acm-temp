...
const Page = () => {
  ...
  const [bgColor, setBgColor] = useState(teamMembers[0].color); // initial background color
    ...
    slides.forEach((slide, idx) => {
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          let opacity = curr > -2500
            ? mapRange(curr, -2500, 0, 0.5, 1)
            : mapRange(curr, -5000, -2500, 0, 0.5);

          slide.style.opacity = `${opacity}`;
          slide.style.transform = `translateX(-50%) translateY(-50%) translateZ(${curr}px)`;

          if (curr < 100) {
            // Change background color when this slide is active
            setBgColor(teamMembers[idx].color);
          }
        },
      });
    });
  ...
  return (
    <div className="min-h-screen container" ref={scrollRef}>
      {/* Single background div controlled by state */}
      <div
        className="active-slide relative w-screen min-h-screen overflow-hidden"
